...
public class TestVocabulary
{
	...
	public enum DatatypeProperty
	{
		...
		private DatatypeProperty(final String localName)
		{
			dp = manager.getOWLDataFactory().getOWLDataProperty(IRI.create(URI_BASE + localName));
		}
	...
	public enum Individual
	{
		...
		private Individual(final String localName)
		{
	...
	{
		IMPORTED_ONTOLOGY("importedOntology"), IMPORTED_ONTOLOGY_IRI("importedOntologyIRI"), PROFILE("profile"), SEMANTICS("semantics"), SPECIES("species"), STATUS("status");

		private final OWLObjectProperty op;

		private ObjectProperty(final String localName)
		{
			op = manager.getOWLDataFactory().getOWLObjectProperty(IRI.create(URI_BASE + localName));
		}
