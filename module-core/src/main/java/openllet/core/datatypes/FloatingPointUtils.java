	...
	public static BigInteger intervalSize(final Double lower, final Double upper)
	{
		...
		if ((DOUBLE_SIGN_MASK & lowerBits) == DOUBLE_SIGN_MASK)
		{
			...
			if ((DOUBLE_SIGN_MASK & upperBits) == DOUBLE_SIGN_MASK)
				return BigInteger.valueOf((DOUBLE_MAGNITUDE_MASK & lowerBits) - (DOUBLE_MAGNITUDE_MASK & upperBits) + 1L);
			/*
			 ...
			 */
				...
				return BigInteger.valueOf(DOUBLE_MAGNITUDE_MASK & lowerBits).add(BigInteger.valueOf(DOUBLE_MAGNITUDE_MASK & upperBits)).add(BigInteger.valueOf(2L));
		}
		/*
		 * Both are positive, size is the difference
		 */
		else
			return BigInteger.valueOf((DOUBLE_MAGNITUDE_MASK & upperBits) - (DOUBLE_MAGNITUDE_MASK & lowerBits) + 1L);
	}
	...
	public static long intervalSize(final Float lower, final Float upper)
	{
		...
		if ((FLOAT_SIGN_MASK & lowerBits) == FLOAT_SIGN_MASK)
		{
			 ...
			 */
				...
				return (long) (FLOAT_MAGNITUDE_MASK & lowerBits) + (long) (FLOAT_MAGNITUDE_MASK & upperBits) + 2L;
		}
		/*
		 * Both are positive, size is the difference
		 */
