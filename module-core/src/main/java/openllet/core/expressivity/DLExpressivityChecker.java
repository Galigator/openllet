...
public class DLExpressivityChecker extends ProfileBasedExpressivityChecker
{
	...
	{
		if (!_KB.getABox().isEmpty()) _expressivity.setHasIndividual(true);
	...
	private void processRoles()
	{
		for (final Role r : _KB.getRBox().getRoles().values())
		{
			...
			if (r.isAnon()) for (final Role subRole : r.getSubRoles())
				if (!subRole.isAnon() && !subRole.isBottom()) _expressivity.setHasInverse(true);
			...
			if (r.isAnon() && r.isFunctional()) _expressivity.setHasInverse(true);
			if (r.isFunctional()) if (r.isDatatypeRole())
				_expressivity.setHasFunctionalityD(true);
			else if (r.isObjectRole()) _expressivity.setHasFunctionality(true);
			if (r.isTransitive()) _expressivity.setHasTransitivity(true);
			if (r.isReflexive()) _expressivity.setHasReflexivity(true);
			if (r.isIrreflexive()) _expressivity.setHasIrreflexivity(true);
			if (r.isAsymmetric()) _expressivity.setHasAsymmetry(true);
			if (!r.getDisjointRoles().isEmpty()) _expressivity.setHasDisjointRoles(true);
			if (r.hasComplexSubRole()) _expressivity.setHasComplexSubRoles(true);
	...
	class Visitor extends ATermBaseVisitor
	{
		...
		public void visitAll(final ATermAppl term)
		{
			...
			if (p instanceof ATermAppl) visitRole((ATermAppl) p);
			visit((ATermAppl) term.getArgument(1));
		}
		...
		public void visitMax(final ATermAppl term)
		{
			...
			else if (cardinality > 1) if (role.isDatatypeRole())
				_expressivity.setHasCardinalityD(true);
			else
				_expressivity.setHasCardinality(true);
		}
