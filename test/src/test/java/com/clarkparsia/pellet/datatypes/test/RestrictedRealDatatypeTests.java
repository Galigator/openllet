...
public class RestrictedRealDatatypeTests
{
	...
	static
	{
		dt = new Datatype<Number>()
		{
			...
			@Override
			public Number getValue(final ATermAppl literal)
			{
				throw new UnsupportedOperationException();
			}
	...
	private static BigDecimal decimal(final String s)
	{
		return new BigDecimal(s);
	}

	private static Rational rational(final Number n, final Number d)
	{
		return new Rational(n, d);
	}

	/**
	 * Verifies the implementation of the contains method for the full real number line. It contains all integer types and BigDecimal, but not the discrete
	...
	/**
	 * Verifies the implementation of the contains method for the full integer number line. It contains all integer types and BigDecimals that are integers, but
	 * not the discrete floating point types or BigDecimals that aren't integers.
	 */
	@Test
	...
	public void containsNoI()
	{
		...
		assertFalse(dr.contains(BigInteger.valueOf(Long.MAX_VALUE).add(BigInteger.valueOf(Long.MAX_VALUE))));
		assertFalse(dr.contains(BigInteger.ZERO.subtract(BigInteger.valueOf(Long.MAX_VALUE).add(BigInteger.valueOf(Long.MAX_VALUE)))));

		assertFalse(dr.contains(BigDecimal.ZERO));
		assertFalse(dr.contains(BigDecimal.ONE));
		assertFalse(dr.contains(BigDecimal.TEN));
		assertFalse(dr.contains(BigDecimal.valueOf(Long.MAX_VALUE).add(BigDecimal.valueOf(1))));
		assertFalse(dr.contains(BigDecimal.valueOf(Long.MIN_VALUE).subtract(BigDecimal.valueOf(2))));
		assertFalse(dr.contains(BigDecimal.valueOf(Long.MAX_VALUE).add(BigDecimal.valueOf(1.1))));
		assertFalse(dr.contains(BigDecimal.valueOf(Long.MIN_VALUE).subtract(BigDecimal.valueOf(0.1))));
	...
	/**
	 * Test that a continuous interval is empty if upper bound is below lower bound
	 *
	...
	/**
	 * Test that a collection of integers is correctly excluded from an infinite continuous interval
	 */
	@Test
	...
	/**
	 * Test that a non-integer is correctly excluded from an infinte continuous interval
	 */
	@Test
	...
	/**
	 * Test that a set of non-integers is correctly excluded from an infinte continuous interval
	 */
	@Test
	...
	/**
	 * Test that a collection of integers is correctly excluded from an infinite integer interval
	 */
	@Test
	...
	public void integerEnumeration2a() throws InvalidConstrainingFacetException
	{
		...
		RestrictedDatatype<Number> neg = new RestrictedRealDatatype(dt, IntegerInterval.allIntegers(), null, null);
	...
	public void integerEnumeration2b() throws InvalidConstrainingFacetException
	{
		...
		RestrictedDatatype<Number> b = new RestrictedRealDatatype(dt, IntegerInterval.allIntegers(), null, null);
	...
	{
		RestrictedDatatype<Number> dr1 = new RestrictedRealDatatype(dt, IntegerInterval.allIntegers(), null, null);
		dr1 = dr1.applyConstrainingFacet(MIN_INCLUSIVE.getName(), -10);
		dr1 = dr1.applyConstrainingFacet(MAX_INCLUSIVE.getName(), -2);

		RestrictedDatatype<Number> dr2 = new RestrictedRealDatatype(dt, IntegerInterval.allIntegers(), null, null);
		dr2 = dr2.applyConstrainingFacet(MIN_INCLUSIVE.getName(), 12);
		dr2 = dr2.applyConstrainingFacet(MAX_INCLUSIVE.getName(), 13);

		final RestrictedDatatype<Number> dr = dr1.union(dr2);
