...
public class OWLRealIntervalTests
{
	...
	public static OWLRealInterval interval(final Double l, final Double u, final boolean il, final boolean iu, final OWLRealInterval.LineType t)
	{
		return new OWLRealInterval(l == null ? null : BigDecimal.valueOf(l), u == null ? null : BigDecimal.valueOf(u), il, iu, t);
	}

	public static OWLRealInterval interval(final Integer l, final Integer u, final boolean il, final boolean iu, final OWLRealInterval.LineType t)
	{
		return new OWLRealInterval(l, u, il, iu, t);
	}

	/**
	 * Verify that exclusive endpoints are not contained within the interval. For continuous intervals.
	 */
	@Test
	...
	/**
	 * Verify that exclusive endpoints are not contained within the interval. For no integer intervals with non-integer bounds.
	 */
	@Test
	public void exclusiveEndpointsNoI1()
	{
		final OWLRealInterval interval = interval(-1.3, 2.5, false, false, LineType.INTEGER_EXCLUDED);
		assertFalse(interval.contains(BigDecimal.valueOf(-1.3d)));
		assertFalse(interval.contains(BigDecimal.valueOf(2.5d)));
	...
	/**
	 * Verify that exclusive endpoints are not contained within the interval. For no integer intervals with integer bounds.
	 */
	@Test
	...
	/**
	 * Verify that exclusive endpoints are not contained within the interval. For integer only intervals with integer bounds.
	 */
	@Test
	...
	/**
	 * Test getting a sub-interval greater than the lower endpoint of an integer line.
	 */
	@Test
	...
	/**
	 * Verify that inclusive endpoints are contained within the interval. For continuous intervals.
	 */
	@Test
	...
	/**
	 * Verify that inclusive endpoints are contained within the interval. For no integer intervals with non-integer bounds.
	 */
	@Test
	...
	/**
	 * Verify that inclusive endpoints are not contained within the interval for no integer intervals with integer bounds.
	 */
	@Test
	...
	/**
	 * Verify that inclusive endpoints are contained within the interval. For integer only intervals with integer bounds.
	 */
	@Test
	...
	/**
	 * Verify that for all interval types, if no overlap exists between the bounds an empty intersection is identified
	 */
	@Test
	public void intersectionEmpty()
	{
		for (final LineType t : LineType.values())
		{
	...
	/**
	 * Verify that if two continuous intervals meet (i.e., one's upper is the other's lower and the (in|ex)clusiveness differs), then they do not intersect.
	 */
	@Test
	...
	/**
	 * Verify that if two no-integer intervals meet (i.e., one's upper is the other's lower and the (in|ex)clusiveness differs), then they do not intersect.
	 */
	@Test
	...
	/**
	 * Verify that the overlap of two continuous intervals with exclusive bounds intersect correctly.
	 */
	@Test
	...
	/**
	 * Verify that the overlap of two continuous intervals with inclusive bounds intersect correctly.
	 */
	@Test
	...
	/**
	 * Verify that two continuous intervals overlapping just on an inclusive bound intersect to a point.
	 */
	@Test
	...
	/**
	 * Verify that two integer intervals overlapping just on an inclusive bound intersect to a point.
	 */
	@Test
	...
	/**
	 * Verify that the intersection of a continuous interval with a continuous interval which it starts and which share an inclusive endpoint is correct.
	 */
	@Test
	...
	/**
	 * Verify that the intersection of a continuous interval with a continuous interval which it starts and which share an exclusive endpoint is correct.
	 */
	@Test
	...
	/**
	 * Verify that the intersection of a continuous interval with a continuous interval which it starts but for which the endpoint type is different it correct
	 * (i.e., it isn't a start relation).
	 */
	@Test
	...
	/**
	 * Test getting a sub-interval less than the upper endpoint of an integer interval
	 */
	@Test
	...
	{
		final OWLRealInterval interval = new OWLRealInterval(null, null, false, false, LineType.CONTINUOUS);
		...
		assertTrue(interval.contains(BigDecimal.valueOf(Long.MAX_VALUE).add(BigDecimal.valueOf(Long.MAX_VALUE)).add(BigDecimal.valueOf(0.123d))));
	}
