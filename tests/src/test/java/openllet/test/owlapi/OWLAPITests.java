...
import static openllet.owlapi.OWL.asymmetric;
import static openllet.owlapi.OWL.classAssertion;
import static openllet.owlapi.OWL.differentFrom;
import static openllet.owlapi.OWL.disjointClasses;
import static openllet.owlapi.OWL.disjointProperties;
import static openllet.owlapi.OWL.equivalentClasses;
import static openllet.owlapi.OWL.equivalentProperties;
import static openllet.owlapi.OWL.functional;
import static openllet.owlapi.OWL.inverse;
import static openllet.owlapi.OWL.inverseFunctional;
import static openllet.owlapi.OWL.irreflexive;
import static openllet.owlapi.OWL.max;
import static openllet.owlapi.OWL.min;
import static openllet.owlapi.OWL.oneOf;
import static openllet.owlapi.OWL.or;
import static openllet.owlapi.OWL.propertyAssertion;
import static openllet.owlapi.OWL.reflexive;
import static openllet.owlapi.OWL.sameAs;
...
public class OWLAPITests extends AbstractOWLAPITests
{
	...
	private static void testOWL2Reasoner(final String ns, final OWLReasoner reasoner)
	{
		...
		assertIteratorValues(reasoner.getTypes(ind1, false).entities().iterator(), OWL.Thing, C, test2, test3);
	...
	public static void assertInstances(final OpenlletReasoner reasoner, final OWLClass subj, final boolean direct, final OWLNamedIndividual... expected)
	{
		assertStreamAsSetEquals(Arrays.asList(expected).stream(), reasoner.getInstances(subj, direct).entities());
	}

	public static void assertPropertyValues(final OpenlletReasoner reasoner, final OWLNamedIndividual subj, final OWLObjectProperty pred, final OWLIndividual... expected)
	{
		assertStreamAsSetEquals(Arrays.asList(expected).stream(), reasoner.getObjectPropertyValues(subj, pred).entities());
	}

	public static void assertPropertyValues(final OpenlletReasoner reasoner, final OWLNamedIndividual subj, final OWLDataProperty pred, final OWLLiteral values)
	{
	...
	public static void assertTypes(final OpenlletReasoner reasoner, final OWLNamedIndividual subj, final boolean direct, final OWLClass... expected)
	{
	...
	public void testPropertyChain()
	{
		...
		assertIteratorValues(reasoner.getInstances(R0, false).entities().iterator(), a[7], a[9]);

		assertIteratorValues(reasoner.getInstances(R1, false).entities().iterator(), a[2], a[3], a[4], a[5], a[6]);

		assertIteratorValues(reasoner.getObjectPropertyValues(a[0], r).entities().iterator(), a[7], a[9]);

		assertIteratorValues(reasoner.getObjectPropertyValues(a[1], r).entities().iterator(), a[2], a[3], a[4], a[5], a[6]);
	...
	@Test
	public void testRemoveLiteralNoBuffering1()
	{
		testRemoveLiteral("ind1", false);
	}

	@Test
	public void testRemoveLiteralNoBuffering2()
	{
		testRemoveLiteral("ind2", false);
	}

	@Test
	public void testRemoveLiteralWithBuffering1()
	{
		testRemoveLiteral("ind1", true);
	}

	@Test
	public void testRemoveLiteralWithBuffering2()
	{
	...
	public void testDLSafeRules()
	{
		...
		assertIteratorValues(reasoner.getTypes(Cain, true).entities().iterator(), BadChild, Child, Person);
	}
	...
	public void testDLSafeConstants()
	{
		...
		for (int test = 0; test < 1; test++)
		{
			...
			assertIteratorValues(reasoner.getInstances(DreamTeamMember2, false).entities().iterator(), Alice, Bob, Charlie);
		}
	}
	...
	{
		final OpenlletReasoner pellet = setupReasonerIndividualNodeSetPolicy(IndividualNodeSetPolicy.BY_SAME_AS);
	...
	{
		final OpenlletReasoner pellet = setupReasonerIndividualNodeSetPolicy(IndividualNodeSetPolicy.BY_NAME);
	...
	{
		final boolean prevValue = OpenlletOptions.HIDE_TOP_PROPERTY_VALUES;
		OpenlletOptions.HIDE_TOP_PROPERTY_VALUES = false;
		try
		{
		...
		{
			OpenlletOptions.HIDE_TOP_PROPERTY_VALUES = prevValue;
		}
	}
	...
	public void testGetEquivalentClasses()
	{
		...
		assertStreamAsSetEquals(Stream.of(_A, _B, _C), _reasoner.equivalentClasses(_A));
		assertStreamAsSetEquals(Stream.of(_A, _B, _C), _reasoner.equivalentClasses(_B));
		assertStreamAsSetEquals(Stream.of(_A, _B, _C), _reasoner.equivalentClasses(_C));
	}
	...
	public void testGetEquivalentObjectProperties()
	{
		...
		assertStreamAsSetEquals(Stream.of(_p, _q, _r), _reasoner.equivalentObjectProperties(_p));
	...
	public void testGetEquivalentDataProperties()
	{
		...
		assertStreamAsSetEquals(Stream.of(_dp, _dq, _dr), _reasoner.equivalentDataProperties(_dp));
		assertStreamAsSetEquals(Stream.of(_dp, _dq, _dr), _reasoner.equivalentDataProperties(_dq));
		assertStreamAsSetEquals(Stream.of(_dp, _dq, _dr), _reasoner.equivalentDataProperties(_dr));
	}
