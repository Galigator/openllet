...
public class ModuleExtractorPersistence
{
	...
	private static String getTypeKeywordForEntity(final OWLEntity owlEntity) throws IllegalArgumentException
	{
		...
		else if (owlEntity.isOWLObjectProperty()) return OBJECT_PROPERTY_KEYWORD;
	...
	private static OWLEntity createEntity(final String type, final String entityIRI)
	{
		...
		else if (OBJECT_PROPERTY_KEYWORD.equals(type)) return OWL.ObjectProperty(entityIRI);
	...
	private static OWLEntity readModuleInformation(final String moduleBeginLine) throws IOException
	{
		...
		if (!tokenizer.hasMoreElements()) throw new IOException("Premature _end of line; module " + MODULE_BEGIN_KEYWORD + " _expected: " + moduleBeginLine);

		tokenizer.nextToken(); // discard the MODULE_BEGIN_KEYWORD

		if (!tokenizer.hasMoreElements()) throw new IOException("Premature _end of line; entity type information _expected: " + moduleBeginLine);

		final String entityType = tokenizer.nextToken();

		if (!tokenizer.hasMoreElements()) throw new IOException("Premature _end of line; module URI _expected: " + moduleBeginLine);

		final String iriString = tokenizer.nextToken();

		if (tokenizer.hasMoreElements()) throw new IOException("Trailing tokens on the line: " + moduleBeginLine);
	...
	private static OWLEntity readModuleMember(final String memberLine) throws IOException
	{
		...
		if (!tokenizer.hasMoreElements()) throw new IOException("Premature _end of line; entity type information _expected: " + memberLine);

		final String entityType = tokenizer.nextToken();

		if (!tokenizer.hasMoreElements()) throw new IOException("Premature _end of line; module member URI _expected: " + memberLine);

		final String iriString = tokenizer.nextToken();

		if (tokenizer.hasMoreElements()) throw new IOException("Trailing tokens on the line: " + memberLine);
	...
	private static boolean readModule(final LineNumberReader lnr, final MultiValueMap<OWLEntity, OWLEntity> modules) throws IOException
	{
		...
		if (!line.startsWith(MODULE_BEGIN_KEYWORD)) throw new IOException("Invalid information in the module file (line " + lnr.getLineNumber() + "). " + MODULE_BEGIN_KEYWORD + " _expected.");
		...
		if (!endOfModuleReached) throw new IOException("Premature _end of file; module information not terminated.");
