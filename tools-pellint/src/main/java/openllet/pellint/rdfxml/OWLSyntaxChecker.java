...
public class OWLSyntaxChecker
{
	 ...
	 */
	private final static Collection<Property>	RESTRICTION_PROPS	= Arrays.asList(OWL.onProperty, OWL.hasValue, OWL.allValuesFrom, OWL.someValuesFrom, OWL.minCardinality,
			OWL2.minQualifiedCardinality, OWL.maxCardinality, OWL2.maxQualifiedCardinality, OWL.cardinality, OWL2.qualifiedCardinality, OWL2.hasSelf);
	...
	private static List<String> toString(final Set<? extends RDFNode> values)
	{
		...
		if (bNodeCount > 0) strings.add(bNodeCount + " BNode(s)");
	...
	{
		if (_lists.containsKey(head)) return;
	...
	private void processTypes()
	{
		...
		for (final Statement stmt : _model.getStatementsByPredicate(RDF.type))
		{
			...
			else if (o.equals(_selfRestriction))
				_OWLEntities.addRestriction(s);
			...
			else if (o.equals(OWL2.NegativePropertyAssertion))
			{
				...
				if (assertedSub != null) _OWLEntities.addIndividual(assertedSub);
				if (assertedPred != null) if (assertedObjTV != null)
				...
				else if (assertedObjTI != null) _OWLEntities.addIndividual(assertedObjTI);
			}
			...
			else if (o.equals(SWRL.ClassAtom) || o.equals(SWRL.DataRangeAtom) || o.equals(SWRL.IndividualPropertyAtom) || o.equals(SWRL.DatavaluedPropertyAtom) || o.equals(SWRL.SameIndividualAtom)
		...
		for (final Statement stmt : processLater)
		{
			...
			{
				if (!_model.containsStatement(s, RDF.type, OWL.Restriction) && !_model.containsStatement(s, RDF.type, OWL.Class)) _OWLEntities.addRDFSClass(s);
			}
			else if (o.equals(OWL.FunctionalProperty))
			{
				if (!_OWLEntities.containsRole(s)) _OWLEntities.assumeObjectRole(s);
			}
			else if (o.equals(RDF.Property))
			{
				if (!_OWLEntities.containsRole(s)) _OWLEntities.assumeObjectRole(s);
			...
			else if (o.equals(SWRL.ClassAtom))
			{
				...
				if (!_OWLEntities.containsIndividual(assertedObject)) _OWLEntities.assumeSWRLVariable(assertedObject);
			}
			else if (o.equals(SWRL.DataRangeAtom))
			{
				...
				if (!assertedObject.isLiteral()) _OWLEntities.assumeSWRLVariable(assertedObject);
			}
			else if (o.equals(SWRL.IndividualPropertyAtom))
			{
				...
				if (!_OWLEntities.containsIndividual(assertedSubject)) _OWLEntities.assumeSWRLVariable(assertedSubject);
				if (!_OWLEntities.containsIndividual(assertedObject)) _OWLEntities.assumeSWRLVariable(assertedObject);
			}
			...
			else if (o.equals(SWRL.SameIndividualAtom) || o.equals(SWRL.DifferentIndividualsAtom))
			{
				...
				if (!_OWLEntities.containsIndividual(assertedObject1)) _OWLEntities.assumeSWRLVariable(assertedObject1);
				if (!_OWLEntities.containsIndividual(assertedObject2)) _OWLEntities.assumeSWRLVariable(assertedObject2);
			}
			else if (o.equals(OWL2.AllDisjointProperties) || o.equals(OWL2.AllDisjointClasses))
			{
	...
	private void processRestrictions()
	{
		for (final Resource res : _OWLEntities.getAllRestrictions())
		{
			...
			if (val != null) if (val.isResource())
				_OWLEntities.addIndividual(val);
			else
				_OWLEntities.assumeDatatypeRole(prop);

			if (!_OWLEntities.containsRole(prop)) _OWLEntities.assumeObjectRole(prop);

			val = _model.getUniqueObject(res, OWL.someValuesFrom);
			if (val == null) val = _model.getUniqueObject(res, OWL.allValuesFrom);
			if (val != null && val.isResource()) if (_OWLEntities.containsObjectRole(prop))
				_OWLEntities.assumeClass(val);
			else if (_OWLEntities.containsDatatypeRole(prop)) _OWLEntities.assumeDatatype(val);
	...
	private void processTriples()
	{
		for (final Statement stmt : _model.getStatements())
		{
			...
			if (o.isLiteral()) _OWLEntities.addLiteral(o);
			...
			else if (p.equals(OWL2.members))
			{
				...
				else if (_model.containsStatement(s, RDF.type, OWL2.AllDisjointClasses))
				{
					if (_lists.containsKey(o))
						for (final RDFNode r : _lists.get(o))
				...
				else if (_model.containsStatement(s, RDF.type, OWL2.AllDisjointProperties))
				{
					if (_lists.containsKey(o))
						for (final RDFNode r : _lists.get(o))
			...
			else if (p.equals(OWL2.assertionProperty) || p.equals(OWL2.targetValue) || p.equals(OWL2.sourceIndividual) || p.equals(OWL2.targetIndividual))
			{
				// processed before
			}
			else if (p.equals(OWL.intersectionOf) || p.equals(OWL.unionOf) || p.equals(OWL2.disjointUnionOf))
			{
			...
			else if (p.equals(OWL2.hasKey))
			{
				...
				if (o.isResource()) if (_lists.containsKey(o))
			...
			else if (p.equals(OWL.equivalentProperty))
			{
				...
				if (!_OWLEntities.containsRole(o)) _OWLEntities.assumeObjectRole(o);
			}
			...
			{
				if (!_OWLEntities.containsRole(s)) if (s.isAnon())
			...
			{
				if (!s.isAnon()) if (s.getURI().equals(Namespaces.RDF.toString()) || s.getURI().equals(Namespaces.OWL.toString()))
			...
			else if (p.equals(RDFS.range))
			{
				...
				if (!_OWLEntities.containsResource(s))
				{
					...
					{
						if (!_OWLEntities.containsRole(s)) _OWLEntities.assumeDatatypeRole(s);
					}
					else if (_OWLEntities.containsClass(o))
					{
						if (!_OWLEntities.containsRole(s)) _OWLEntities.assumeObjectRole(s);
					}
					else if (_OWLEntities.containsIndividual(o) || _OWLEntities.containsRole(o))
					{
				...
				else if (!_OWLEntities.containsResource(o)) if (_OWLEntities.containsObjectRole(s))
					_OWLEntities.assumeClass(o);
				else if (_OWLEntities.containsDatatypeRole(s)) _OWLEntities.assumeDatatype(o);
			}
			...
			else if (_OWLEntities.containsOntologyRole(p))
			{
				_OWLEntities.assumeOntology(o);
				_OWLEntities.assumeOntology(s);

			}
			else if (p.equals(SWRL.Imp) || p.equals(SWRL.head) || p.equals(SWRL.body) || p.equals(SWRL.builtin))
			{
				// Ignore
			}
			else if (p.equals(SWRL.classPredicate) || p.equals(SWRL.propertyPredicate) || p.equals(SWRL.argument1) || p.equals(SWRL.argument2) || p.equals(SWRL.arguments))
			{
				// Processed before
			}
			else
			{
				if (_OWLEntities.containsAnnotaionRole(p))
					continue;
				else if (!_OWLEntities.containsRole(p)) if (o.isLiteral())
					_OWLEntities.assumeDatatypeRole(p);
				else if (!_OWLEntities.containsIndividual(s))
					_OWLEntities.assumeAnnotationRole(p);
				else
					_OWLEntities.assumeObjectRole(p);

				if (_OWLEntities.containsAnnotaionRole(p)) continue;
				...
				if (_OWLEntities.containsDatatypeRole(p))
				{
					if (o.isLiteral())
					{
						...
						if (datatypeURI != null && !datatypeURI.equals(""))
						{
							...
							if (!_OWLEntities.containsDatatype(datatype)) _OWLEntities.assumeDatatype(datatype);
						}
					}
					else
